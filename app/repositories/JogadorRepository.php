...
{
    $sql = "UPDATE jogadores SET nome = :nome, data_nascimento = :data, nacionalidade = :nac, posicao = :pos, altura = :alt, peso = :peso, pe_dominante = :pe, time = :time, imagem = :img 
            WHERE id = :id";

    $stmt = $this->connection->prepare($sql);
    
    $stmt->bindValue(':nome', $jogador->getNome());
    $stmt->bindValue(':data', $jogador->getDataNascimento());
    $stmt->bindValue(':nac', $jogador->getNacionalidade());
    $stmt->bindValue(':pos', $jogador->getPosicao());
    $stmt->bindValue(':alt', $jogador->getAltura());
    $stmt->bindValue(':peso', $jogador->getPeso());
    $stmt->bindValue(':pe', $jogador->getPeDominante());
    $stmt->bindValue(':time', $jogador->getTime());
    $stmt->bindValue(':img', $jogador->getImagem());
